...
func canSolve(input: Int) -> Bool {

    //input is the number of problems solved in the first hour
    //there are twelve problems total
    //each problem after the first hour takes 45 minutes
    //there are 5 hours (300 minutes) total
    
    let remainingProblems = 12 - input
    let timeRequired = remainingProblems * 45
    if timeRequired <= (300 - 60) {
        return true
    } else {
        return false
    }
...
func aboutGrisha(input: Int) -> String {

    return boolToString(canSolve(input))
}

print(aboutGrisha(input))
print(aboutGrisha(input2))
