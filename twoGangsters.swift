...
func twoGangsters(input: String) -> String {


// input

// The String manipulation is Swift 2.1 syntax
let array = input.characters.split(" ") // creates array of Characters
let arrayOfStrings = array.map {String($0)} // turn each  Character into String? (in order to cast to Int)
let arrayOfInts = arrayOfStrings.map {Int($0)!} // unwrap each string and cast to Int

let harryShot = Int(arrayOfInts[0])
let larryShot = Int(arrayOfInts[1]) 

// Determine the total number of cans that were shot.
let totalCansShot = harryShot + larryShot - 1 // one of the cans was shot by both

let harryNotShot = totalCansShot - harryShot
let larryNotShot = totalCansShot - larryShot

return "\(harryNotShot) \(larryNotShot)"

}

print(twoGangsters(input))
